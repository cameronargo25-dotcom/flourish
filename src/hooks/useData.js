...
export function useData(userId) {
  ...
  async function saveVideo(video) {
    if (video.id) {
      ...
      const { data, error } = await supabase
        .from('videos').update(updates).eq('id', video.id).select().single()
      if (!error && data) setVideos(prev => prev.map(v => v.id === data.id ? data : v))
      return { error }
    } else {
      const { data, error } = await supabase
        .from('videos').insert({ ...video, user_id: userId }).select().single()
  ...
  async function moveVideo(id, stage) {
    const { data, error } = await supabase
      .from('videos').update({ stage }).eq('id', id).select().single()
    if (!error && data) setVideos(prev => prev.map(v => v.id === data.id ? data : v))
    return { error }
  }
  ...
  async function unarchiveVideo(id) {
    const { data, error } = await supabase
      .from('videos').update({ archived: false, stage: 3 }).eq('id', id).select().single()
    if (!error && data) setVideos(prev => prev.map(v => v.id === data.id ? data : v))
    return { error }
  }
  ...
  async function saveSample(sample) {
    if (sample.id) {
      ...
      const { data, error } = await supabase
        .from('samples').update(updates).eq('id', sample.id).select().single()
      if (!error && data) setSamples(prev => prev.map(s => s.id === data.id ? data : s))
      return { error }
    } else {
      const { data, error } = await supabase
        .from('samples').insert({ ...sample, user_id: userId }).select().single()
  ...
  async function saveTopVideo(tv) {
    if (tv.id) {
      ...
      const { data, error } = await supabase
        .from('top_videos').update(updates).eq('id', tv.id).select().single()
      if (!error && data) setTopVideos(prev => prev.map(t => t.id === data.id ? data : t))
      return { error }
    } else {
      const { data, error } = await supabase
        .from('top_videos').insert({ ...tv, user_id: userId }).select().single()
