...
export default function Home({ profile, goals, videos, samples, topVideos, onSaveProfile, onSaveTopVideo, onDeleteTopVideo }) {
  const [editingProfile, setEditingProfile] = useState(false)
  const [editingTopVideo, setEditingTopVideo] = useState(null) // null | {} | {id,..}
  ...
  async function handleSaveTv() {
    await onSaveTopVideo(tvForm)
    ...
    <div style={styles.page}>
      ...
      <div style={styles.profileCard}>
        ...
        <div style={styles.identity}>
          ...
          <button style={{ ...styles.editBtn, borderColor: accent.color, color: accent.color }} onClick={openProfileEdit}>
            Edit profile
          </button>
        </div>
      </div>
      ...
      <div style={styles.topVidsGrid}>
        ...
        {topVideos.length === 0 && (
          <div style={styles.emptyVids}>Add your best performing videos to showcase them here.</div>
        )}
      </div>
            ...
            <div style={{ display: 'flex', gap: 8, marginTop: 4 }}>
              {ACCENTS.map(a => (
                <div key={a.name} onClick={() => setForm(f => ({ ...f, accent_color: a.name }))}
                  style={{ width: 26, height: 26, borderRadius: '50%', background: a.color, cursor: 'pointer',
...
const styles = {
  ...
  banner: { height: 60 },
  identity: { padding: '0 16px 14px', display: 'flex', alignItems: 'flex-end', gap: 12, marginTop: -22 },
  ...
  editBtn: { fontSize: 12, padding: '5px 12px', borderRadius: 8, border: '0.5px solid', background: 'none', cursor: 'pointer', flexShrink: 0, marginBottom: 4 },
  statsGrid: { display: 'grid', gridTemplateColumns: 'repeat(4, minmax(0,1fr))', gap: 10, marginBottom: 14 },
  pipelineGrid: { display: 'grid', gridTemplateColumns: 'repeat(3, minmax(0,1fr))', gap: 10, marginBottom: 14 },
  statCard: { background: '#f8f7f5', borderRadius: 8, padding: '11px 13px' },
  ...
  snapRow: { display: 'flex', alignItems: 'center', gap: 10, padding: '8px 0', borderBottom: '0.5px solid rgba(0,0,0,0.06)' },
  snapTitle: { fontSize: 13, fontWeight: '500', flex: 1, minWidth: 0, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' },
  snapTrack: { flex: '0 0 120px', height: 5, background: '#eee', borderRadius: 3, overflow: 'hidden' },
  snapFill: { height: '100%', borderRadius: 3 },
  snapPct: { fontSize: 12, fontWeight: '500', flex: '0 0 36px', textAlign: 'right' },
  topVidsGrid: { display: 'grid', gridTemplateColumns: 'repeat(3, minmax(0,1fr))', gap: 10, marginBottom: 14 },
  ...
  emptyVids: { fontSize: 13, color: '#aaa', padding: '20px 0', gridColumn: '1/-1' },
  smBtn: { fontSize: 11, padding: '3px 10px', borderRadius: 8, border: '0.5px solid rgba(0,0,0,0.2)', background: 'none', cursor: 'pointer', color: '#666', marginBottom: 10 },
