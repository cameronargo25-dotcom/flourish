...
export default function Pipeline({ videos, onSave, onDelete, onMove }) {
  ...
  function openNew(stage = 0) {
    setForm({
      title: '', type: 'shop', stage, deadline: '',
      affiliate_link: '', commission: '', caption: '',
      hashtags: '', notes: '', platforms: [], other_platform: '',
    })
    setEditing({})
  }

  function openEdit(v) {
    setForm({ ...v, platforms: v.platforms || [], other_platform: v.other_platform || '' })
    setEditing(v)
  }
    ...
    <div style={styles.page}>
                ...
                return (
                  ...
                  >
                    ...
                    <div style={styles.cardMeta}>
                      ...
                      {dlLabel && (
                        <span style={{ fontSize: 11, color: dlColor, fontWeight: dlColor === '#993C1D' ? '500' : '400' }}>
                          {dlLabel}
                        </span>
                      )}
                    </div>
                    {platforms.length > 0 && (
                      <div style={styles.platformRow}>
                        {platforms.map(pid => {
                          ...
                          return (
                            <span key={pid} style={{ ...styles.platformPill, background: col.bg, color: col.color }}>
                              {platformLabel(pid, v.other_platform)}
                            </span>
                          )
                        })}
                      </div>
      ...
      {editing !== null && (
        ...
        >
          <Row>
            <Field label="Title">
              <input style={inputStyle} value={form.title || ''} onChange={e => setForm(f => ({ ...f, title: e.target.value }))} placeholder="Video title" />
            </Field>
            <Field label="Type">
          ...
          <Row>
            <Field label="Affiliate link">
              <input style={inputStyle} value={form.affiliate_link || ''} onChange={e => setForm(f => ({ ...f, affiliate_link: e.target.value }))} placeholder="https://..." />
            </Field>
            <Field label="Commission">
              <input style={inputStyle} value={form.commission || ''} onChange={e => setForm(f => ({ ...f, commission: e.target.value }))} placeholder="e.g. 8%" />
            </Field>
          </Row>
          <Row cols={1}>
          ...
          <Row cols={1}>
            <Field label="Hashtags">
              <input style={inputStyle} value={form.hashtags || ''} onChange={e => setForm(f => ({ ...f, hashtags: e.target.value }))} placeholder="#tiktokmademebuyit ..." />
            </Field>
          </Row>
          <Row cols={1}>
            <Field label="Notes">
